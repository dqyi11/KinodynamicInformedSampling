...
namespace Eigen
{
    ...
    private:
        ...
        void deflation44(Index firstColu, Index firstColm, Index firstRowW, Index firstColW, Index i, Index j,
                         Index size);
...

        {
            ...
            std::cout << "    check3 (>0)      : "
                      << ((diag.segment(1, actual_n - 1) - singVals.head(actual_n - 1).array()) /
                          singVals.head(actual_n - 1).array()).transpose() << "\n\n\n";
            std::cout << "    check4 (>0)      : "
...
//   "\n";
                                             ...
                                             VectorType &singVals, ArrayRef shifts, ArrayRef mus)
    {
        ...
        for (Index k = 0; k < n; ++k)
        {
            ...
            while (fCur != 0 &&
                   abs(muCur - muPrev) >
                       8 * NumTraits<RealScalar>::epsilon() * numext::maxi<RealScalar>(abs(muCur), abs(muPrev)) &&
            ...
            if (useBisection)
            {
                ...
                if (shift == left)
                {
                    ...
                    rightShifted =
                        (k == actual_n - 1) ? right : ((right - left) * RealScalar(0.6));  // theoretically we can
            ...
            else
            {
                ...
                for (Index l = 0; l < m; ++l)
                {
                    ...
                    if (i != k)
                    {
                        ...
                        prod *=
                            ((singVals(j) + dk) / ((diag(i) + dk))) * ((mus(j) + (shifts(j) - dk)) / ((diag(i) - dk)));
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
                        if (i != k &&
                            ...
                            std::cout << "     "
                                      << ((singVals(j) + dk) * (mus(j) + (shifts(j) - dk))) /
                                             ((diag(i) + dk) * (diag(i) - dk)) << " == (" << (singVals(j) + dk) << " * "
                                       ...
                                       Index shift)
    {
        ...
        for (Index i = 1; i < length; ++i)
            ...
            {
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
                std::cout << "deflation 4.2, set z(" << i << ") to zero because " << abs(col0(i)) << " < "
                          << epsilon_strict << "  (diag(" << i << ")=" << diag(i) << ")\n";
#endif
                col0(i) = 0;
            }
        ...
        for (Index i = 1; i < length; i++)
            ...
            {
#ifdef EIGEN_BDCSVD_DEBUG_VERBOSE
                std::cout << "deflation 4.3, cancel z(" << i << ")=" << col0(i) << " because diag(" << i
                          << ")=" << diag(i) << " < " << epsilon_coarse << "\n";
#endif
...
#endif
                    eigen_internal_assert(abs(diag(i) - diag(i - 1)) < epsilon_coarse && " diagonal entries are not "
                                                                                         "properly sorted");
