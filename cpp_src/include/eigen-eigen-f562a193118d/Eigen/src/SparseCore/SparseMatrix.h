...
namespace Eigen
{
    ...
    template <typename _Scalar, int _Options, typename _Index>
    class SparseMatrix : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _Index>>
    {
    ...
    public:
        ...
        {
            eigen_assert(Index(m_outerIndex[outer + 1]) == m_data.size() && "Invalid ordered insertion (invalid outer "
                                                                            "index)");
            eigen_assert(
        ...
        {
            eigen_assert(m_outerIndex[outer] == Index(m_data.size()) && "You must call startVec for each inner vector "
                                                                        "sequentially");
          ...
          : m_outerSize(0), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0)
        {
            ...
            const bool needToTranspose =
                (Flags & RowMajorBit) != (internal::evaluator<OtherDerived>::Flags & RowMajorBit);
            if (needToTranspose)
                *this = other.derived();
            else
            {
...
#include EIGEN_SPARSEMATRIX_PLUGIN
    ...
    {
        internal::set_from_triplets<InputIterators, SparseMatrix<Scalar, _Options, _Index>>(
    ...
    void SparseMatrix<Scalar, _Options, _Index>::setFromTriplets(const InputIterators &begin, const InputIterators &end,
                                                                 DupFunctor dup_func)
    {
        internal::set_from_triplets<InputIterators, SparseMatrix<Scalar, _Options, _Index>, DupFunctor>(
    ...
    template <typename _Scalar, int _Options, typename _Index>
    typename SparseMatrix<_Scalar, _Options, _Index>::Scalar &SparseMatrix<_Scalar, _Options, _Index>::insert(Index row,
                                                                                                              Index col)
    {
        eigen_assert(row >= 0 && row < rows() && col >= 0 && col < cols());
    ...
    SparseMatrix<_Scalar, _Options, _Index>::insertCompressed(Index row, Index col)
    {
        ...
        bool isLastVec =
            (!(previousOuter == -1 && m_data.size() != 0)) && (size_t(m_outerIndex[outer + 1]) == m_data.size());
