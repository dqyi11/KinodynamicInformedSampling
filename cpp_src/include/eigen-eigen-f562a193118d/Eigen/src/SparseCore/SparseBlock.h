...
namespace Eigen
{
    ...
    ***************************************************************************/
        ...
        public:
            ...
            inline BlockType &operator=(const SparseMatrixBase<OtherDerived> &other)
            {
                ...
                Ref < const SparseMatrix < Scalar, IsRowMajor ? RowMajor : ColMajor,
                    StorageIndex >> tmp(other.derived());
                eigen_internal_assert(tmp.outerSize() == m_outerSize.value());

                // 2 - let's check whether there is enough allocated memory
                Index nnz = tmp.nonZeros();
                Index start = m_outerStart == 0 ? 0 : m_matrix.outerIndexPtr()[m_outerStart];  // starting position of
                                                                                               // the current block
                Index end = m_matrix.outerIndexPtr()[m_outerStart + m_outerSize.value()];      // ending
                                                                                               // position of
                                                                                               // the current
                                                                                               // block
                ...
                Index free_size =
                    m_matrix.isCompressed() ? Index(matrix.data().allocatedSize()) + block_size : block_size;
                ...
                if (nnz > free_size)
                {
                    ...
                    internal::smart_copy(m_matrix.innerIndexPtr(), m_matrix.innerIndexPtr() + start,
                                         newdata.indexPtr());
    ...
    class BlockImpl<SparseMatrix<_Scalar, _Options, _StorageIndex>, BlockRows, BlockCols, true, Sparse>
        : public internal::sparse_matrix_block_impl<SparseMatrix<_Scalar, _Options, _StorageIndex>, BlockRows,
                                                    BlockCols>
    {
    public:
    ...
    typename SparseMatrixBase<Derived>::InnerVectorsReturnType SparseMatrixBase<Derived>::innerVectors(Index outerStart,
                                                                                                       Index outerSize)
    {
        return Block<Derived, Dynamic, Dynamic, true>(derived(), IsRowMajor ? outerStart : 0,
    ...
    protected:
        ...
        friend struct internal::unary_evaluator<Block<XprType, BlockRows, BlockCols, InnerPanel>,
    ...
    namespace internal
    {
        ...
        public:
            ...
            enum
            {
                ...
                OuterVector = (BlockCols == 1 && ArgType::IsRowMajor) |  // FIXME | instead of || to please GCC
                                                                         // 4.4.0 stupid warning "suggest
                                                                         // parentheses around &&".
        ...
        public:
            EIGEN_STRONG_INLINE InnerVectorInnerIterator(const unary_evaluator &aEval, Index outer)
              : EvalIterator(aEval.m_argImpl,
                             outer + (IsRowMajor ? aEval.m_block.startRow() : aEval.m_block.startCol()))
            ...
            inline StorageIndex index() const
            {
                return EvalIterator::index() -
                       convert_index<StorageIndex>(IsRowMajor ? m_block.startCol() : m_block.startRow());
            }
        ...
        public:
            EIGEN_STRONG_INLINE OuterVectorInnerIterator(const unary_evaluator &aEval, Index outer)
              : m_eval(aEval)
              , m_outerPos((IsRowMajor ? aEval.m_block.startCol() : aEval.m_block.startRow()) - 1)
              ,  // -1 so that operator++ finds the first non-zero entry
            ...
            inline OuterVectorInnerIterator &operator++()
            {
                ...
                while (++m_outerPos < m_end)
                {
                    ...
                    while (it && it.index() < m_innerIndex)
                        ++it;
