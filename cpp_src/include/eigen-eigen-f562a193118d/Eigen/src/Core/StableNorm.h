    ...
    inline typename NumTraits<typename internal::traits<Derived>::Scalar>::Real MatrixBase<Derived>::stableNorm() const
    {
        ...
        enum
        {
            CanAlign =
                ...
                (blockSize * sizeof(Scalar) * 2 < EIGEN_STACK_ALLOCATION_LIMIT)  // ifwe cannot allocate on the
                                                                                 // stack, then let's not bother
                                                                                 // about this optimization
        };
        typedef typename internal::conditional<CanAlign, Ref<const Matrix<Scalar, Dynamic, 1, 0, blockSize, 1>,
