...
*****************************************************************************/
        ...
        EIGEN_DEVICE_FUNC inline void check_that_malloc_is_allowed()
        {
            eigen_assert(is_malloc_allowed() && "heap allocation is forbidden "
                                                "(EIGEN_RUNTIME_NO_MALLOC is defined and "
        ...
        *****************************************************************************/
         ...
         * T.
          */
        template <typename T>
        EIGEN_DEVICE_FUNC inline T *construct_elements_of_array(T *ptr, size_t size)
        {
            size_t i;
            EIGEN_TRY
            {
...
#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE, BUFFER)                                        \
    ...
    TYPE *NAME =                                                                                                       \
        (BUFFER) != 0 ? (BUFFER) : reinterpret_cast<TYPE *>((sizeof(TYPE) * SIZE <= EIGEN_STACK_ALLOCATION_LIMIT) ?    \
