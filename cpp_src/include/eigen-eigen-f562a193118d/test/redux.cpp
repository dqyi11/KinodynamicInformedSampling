...
void matrixRedux(const MatrixType &m)
{
    ...
    VERIFY_IS_APPROX(MatrixType::Ones(rows, cols).sum(), Scalar(float(rows * cols)));  // the float() here to shut up
                                                                                       // excessive MSVC warning about
                                                                                       // int->complex conversion being
                                                                                       // lossy
    ...
    VERIFY_EVALUATION_COUNT((m1.matrix() * m1.matrix().transpose()).sum(),
                            (MatrixType::SizeAtCompileTime == Dynamic ? 1 : 0));
