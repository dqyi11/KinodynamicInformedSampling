...
void vectorwiseop_matrix(const MatrixType &m)
{
    ...
    VERIFY_EVALUATION_COUNT(tmp = (m1 * m1.transpose()).colwise().sum(),
                            (MatrixType::RowsAtCompileTime == Dynamic ? 1 : 0));
    ...
    VERIFY_EVALUATION_COUNT(m2 = (m1.rowwise() - m1.colwise().sum() / RealScalar(m1.rows())),
                            (MatrixType::RowsAtCompileTime == Dynamic && MatrixType::ColsAtCompileTime != 1 ? 1 : 0));
}
