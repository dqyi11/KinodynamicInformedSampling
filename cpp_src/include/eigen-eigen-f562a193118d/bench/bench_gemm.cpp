...
void blas_gemm(const MatrixXf &a, const MatrixXf &b, MatrixXf &c)
{
    ...
    sgemm_(&notrans, &notrans, &M, &N, &K, &fone, const_cast<float *>(a.data()), &lda, const_cast<float *>(b.data()),
           &ldb, &fone, c.data(), &ldc);
}
...
EIGEN_DONT_INLINE void blas_gemm(const MatrixXd &a, const MatrixXd &b, MatrixXd &c)
{
    ...
    dgemm_(&notrans, &notrans, &M, &N, &K, &done, const_cast<double *>(a.data()), &lda, const_cast<double *>(b.data()),
           &ldb, &done, c.data(), &ldc);
}
...
void blas_gemm(const MatrixXcf &a, const MatrixXcf &b, MatrixXcf &c)
{
    ...
    cgemm_(&notrans, &notrans, &M, &N, &K, (float *)&cfone, const_cast<float *>((const float *)a.data()), &lda,
...
void blas_gemm(const MatrixXcd &a, const MatrixXcd &b, MatrixXcd &c)
{
    ...
    zgemm_(&notrans, &notrans, &M, &N, &K, (double *)&cdone, const_cast<double *>((const double *)a.data()), &lda,
