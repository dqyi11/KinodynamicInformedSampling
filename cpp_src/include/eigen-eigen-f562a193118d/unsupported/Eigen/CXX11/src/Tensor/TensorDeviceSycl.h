...
namespace Eigen
{
    struct SyclDevice
    {
        ...
        /// SyclDevice.
        /// If a non-read-only pointer is needed to be accessed on the host we should
        ...
        template <typename T>
        inline std::pair<std::map<const void *, std::shared_ptr<void>>::iterator, bool>
        add_sycl_buffer(const T *ptr, size_t num_bytes) const
        {
            ...
            std::pair<std::map<const void *, std::shared_ptr<void>>::iterator, bool> ret =
                buffer_map.insert(std::pair<const void *, std::shared_ptr<void>>(
                    ptr, std::shared_ptr<void>(new Type(cl::sycl::range<1>(num_bytes)), [](void *dataMem)
                                               {
