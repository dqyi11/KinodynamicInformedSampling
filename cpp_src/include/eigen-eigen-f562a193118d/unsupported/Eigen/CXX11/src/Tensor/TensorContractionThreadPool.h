              ...
              TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, ThreadPoolDevice>>
    {
        ...
        void evalProduct(Scalar *buffer) const
        {
            ...
            typedef internal::TensorContractionInputMapper<
                LhsScalar, Index, internal::Lhs, LeftEvaluator, left_nocontract_t, contract_t,
                internal::packet_traits<LhsScalar>::size, lhs_inner_dim_contiguous, false, Unaligned> LhsMapper;
            typedef internal::TensorContractionInputMapper<
                RhsScalar, Index, internal::Rhs, RightEvaluator, right_nocontract_t, contract_t,
                internal::packet_traits<RhsScalar>::size, rhs_inner_dim_contiguous, rhs_inner_dim_reordered,
                Unaligned> RhsMapper;
            ...
            typedef internal::gemm_pack_rhs<RhsScalar, Index, typename RhsMapper::SubMapper, Traits::nr, ColMajor>
                RhsPacker;
            typedef internal::gebp_kernel<LhsScalar, RhsScalar, Index, OutputMapper, Traits::mr, Traits::nr, false,
                                          false> GebpKernel;
            ...
            if (shard_by_col)
            {
                internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByCol> blocking(k, m, n,
                                                                                                                2);
                bm = blocking.mc();
                bn = blocking.nc();
                bk = blocking.kc();
            ...
            {
                internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByRow> blocking(k, m, n,
                                                                                                                2);
                bm = blocking.mc();
                bn = blocking.nc();
                bk = blocking.kc();
            ...
            int num_threads = TensorCostModel<ThreadPoolDevice>::numThreads(static_cast<double>(n) * m, cost,
            ...
            if (num_threads == 1)
            {
                ...
                if (n == 1)
                    this->template evalGemv<lhs_inner_dim_contiguous, rhs_inner_dim_contiguous, rhs_inner_dim_reordered,
                                            Alignment>(buffer);
                else
            ...
            Context<LhsPacker, RhsPacker, GebpKernel, LhsMapper, RhsMapper, OutputMapper>(
                this->m_device, num_threads, lhs, rhs, buffer, m, n, k, bm, bn, bk, nm, nn, nk, gm, gn, nm0, nn0,
                shard_by_col, parallel_pack).run();
        }

        // Context coordinates a single parallel gemm operation.
        template <typename LhsPacker, typename RhsPacker, typename GebpKernel, typename LhsMapper, typename RhsMapper,
                  typename OutputMapper>
        class Context
        {
        public:
            Context(const Device &device, int num_threads, LhsMapper &lhs, RhsMapper &rhs, Scalar *buffer, Index tm,
                    Index tn, Index tk, Index bm, Index bn, Index bk, Index nm, Index nn, Index nk, Index gm, Index gn,
                    Index nm0, Index nn0, bool shard_by_col, bool parallel_pack)
              : device_(device)
              , lhs_(lhs)
              , rhs_(rhs)
              , buffer_(buffer)
              ...
              , nn0_(nn0)
            {
                for (Index x = 0; x < P; x++)
                {
                    ...
                    state_switch_[x] = x == 0 ? 1 : (parallel_pack_ ? nn_ + nm_ : (shard_by_col_ ? nn_ : nm_)) +
                                                        (x == P - 1 ? nm_ * nn_ : 0);
        ...
        private:
            ...
            void kernel(Index m, Index n, Index k)
            {
                ...
                if (shard_by_col_)
                {
                    ...
                    {
                        for (Index m1 = m * gm_; m1 < mend; m1++)
                            GebpKernel()(output_.getSubMapper(m1 * bm_, n1 * bn_), packed_lhs_[k % (P - 1)][m1],
                                         packed_rhs_[k % (P - 1)][n1], bm(m1), bk(k), bn(n1), Scalar(1), -1, -1, 0, 0);
                    }
                }
                else
                {
        ...
        Index coarsenM(Index m, Index n, Index bm, Index bn, Index bk, Index gn, int num_threads,
        ...
        Index coarsenN(Index m, Index n, Index bm, Index bn, Index bk, Index gm, int num_threads,
                       bool shard_by_col) const
        {
            Index gn = 1;
            Index gn1 = 1;
                       ...
                       int num_threads, bool shard_by_col) const
        {
            ...
            double new_parallelism =
                static_cast<double>(new_tasks) / (divup<int>(new_tasks, num_threads) * num_threads);
            Index old_tasks = divup(nm0, oldgm) * divup(nn0, oldgn);
            double old_parallelism =
                static_cast<double>(old_tasks) / (divup<int>(old_tasks, num_threads) * num_threads);
        ...
        void evalGemm(Scalar *buffer) const
        {
            ...
            typedef internal::TensorContractionInputMapper<LhsScalar, Index, internal::Lhs, LeftEvaluator,
                                                           left_nocontract_t, contract_t, lhs_packet_size,
                                                           lhs_inner_dim_contiguous, false, Unaligned> LhsMapper;

            typedef internal::TensorContractionInputMapper<
                RhsScalar, Index, internal::Rhs, RightEvaluator, right_nocontract_t, contract_t, rhs_packet_size,
                rhs_inner_dim_contiguous, rhs_inner_dim_reordered, Unaligned> RhsMapper;
                ...
                for (Index m_block_idx = 0; m_block_idx < m_blocks; m_block_idx += numBlockAs)
                {
                    ...
                    for (Index n_block_idx = 0; n_block_idx < n_blocks; n_block_idx++)
                    {
                        ...
                        packRKArg arg = {
                            ...
                            actual_kc,             // kc
                            actual_nc,             // nc
                            num_threads, numBlockAs, m, k_block_idx, m_block_idx,
                        ...
                        this->m_device.enqueueNoNotification(&Self::packRhsAndKernel<packRKArg, RhsPacker, GebpKernel>,
                                                             arg);
                    }
                }
            }
...
#undef CEIL_DIV
        ...
        static void packRhsAndKernel(packRKArg arg)
        {
            ...
            for (Index mt_block_idx = 0; mt_block_idx < arg.num_blockAs; mt_block_idx++)
            {
                ...
                {
                    Index blockAId =
                        (arg.k_block_idx * arg.m_blocks + mt_block_idx + arg.m_block_idx) % arg.num_threads;
                    ...
                    gebp(arg.output.getSubMapper(m_base_start, arg.n), (*arg.blockAs)[blockAId], arg.blockB, actual_mc,
                         arg.kc, arg.nc, Scalar(1), -1, -1, 0, 0);
                                     ...
                                     bool prepacked) const
        {
            const int packed_size =
                std::min<int>(PacketType<LhsScalar, Device>::size, PacketType<RhsScalar, Device>::size);
            ...
            double computeBandwidth =
                bk == 1 ? 4.0 : (shard_by_col ? bn : bm) < Traits::nr || (shard_by_col ? bm : bn) < Traits::mr ? 2.0 :
                                                                                                                 0.5;
