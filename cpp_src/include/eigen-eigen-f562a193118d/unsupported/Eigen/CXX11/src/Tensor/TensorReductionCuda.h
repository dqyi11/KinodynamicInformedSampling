...
namespace Eigen
{
    namespace internal
    {
        ...
        // and
        // attempts to update it with the new value. If in the meantime another cuda
        // thread
...
#pragma unroll
        ...
        struct FullReductionLauncher<Self, Op, OutputType, PacketAccess,
                                                                  ...
                                                                  void>::type>
        {
                            ...
                            typename Self::Index num_coeffs)
            {
                ...
                LAUNCH_CUDA_KERNEL((FullReductionKernel<block_size, num_per_thread, Self, Op, Index>), num_blocks,
                                   block_size, 0, device, reducer, self, num_coeffs, output, semaphore);
            }
        };
        ...
        struct FullReductionLauncher<Self, Op, Eigen::half, true>
        {
                            ...
                            typename Self::Index num_coeffs)
            {
                ...
                if (num_blocks > 1)
                {
                    ...
                    LAUNCH_CUDA_KERNEL((ReductionInitFullReduxKernelHalfFloat<Self, Op, Index>), 1, 1, 0, device,
                                       reducer, self, num_coeffs, scratch);
                }
                ...
                if (num_blocks > 1)
                {
                    LAUNCH_CUDA_KERNEL((ReductionCleanupKernelHalfFloat<Op>), 1, 1, 0, device, reducer, output,
                                       scratch);
                }
            }
        };
...
// of doubles, floats and half floats
            ...
            static const bool HasOptimizedImplementation =
                !Op::IsStateful && (internal::is_same<typename Self::CoeffReturnType, float>::value ||
...
#pragma unroll
        ...
        // Specialization for float and double
        template <typename Self, typename Op, typename OutputType, bool PacketAccess>
        struct InnerReductionLauncher<Self, Op, OutputType, PacketAccess,
                                      typename internal::enable_if<internal::is_same<float, OutputType>::value ||
                                                                       internal::is_same<double, OutputType>::value,
                                                                   void>::type>
        {
                            ...
                            typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals)
            {
                ...
                const int max_blocks =
                    device.getNumCudaMultiProcessors() * device.maxCudaThreadsPerMultiProcessor() / block_size;
                ...
                if (num_blocks > 1)
                {
                    ...
                    const int max_blocks =
                        device.getNumCudaMultiProcessors() * device.maxCudaThreadsPerMultiProcessor() / 1024;
                    const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
                    LAUNCH_CUDA_KERNEL((ReductionInitKernel<OutputType, Index>), num_blocks, 1024, 0, device,
                                       reducer.initialize(), num_preserved_vals, output);
                }

                LAUNCH_CUDA_KERNEL((InnerReductionKernel<num_per_thread, Self, Op, Index>), num_blocks, block_size, 0,
                            ...
                            typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals)
            {
                ...
                if (num_blocks > 1)
                {
                    ...
                    const int max_blocks =
                        device.getNumCudaMultiProcessors() * device.maxCudaThreadsPerMultiProcessor() / 1024;
                    const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
                    LAUNCH_CUDA_KERNEL((ReductionInitKernelHalfFloat<Self, Op, Index>), 1, 1, 0, device, reducer, self,
                                       num_preserved_vals, output);
                }

                LAUNCH_CUDA_KERNEL((InnerReductionKernelHalfFloat<num_per_thread, Self, Op, Index>), num_blocks,
                                   block_size, 0, device, reducer, self, num_coeffs_to_reduce, num_preserved_vals,
                                   output);
...
// of floats and half floats.
        ...
        struct OuterReducer<Self, Op, GpuDevice>
        {
            ...
            static EIGEN_DEVICE_FUNC bool run(const Self &, Op &, const Device &, OutputType *, typename Self::Index,
                                              typename Self::Index)
            {
                            ...
                            typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals)
            {
                ...
                if (num_blocks > 1)
                {
                    ...
                    const int max_blocks =
                        device.getNumCudaMultiProcessors() * device.maxCudaThreadsPerMultiProcessor() / 1024;
                    const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
                    LAUNCH_CUDA_KERNEL((ReductionInitKernel<float, Index>), num_blocks, 1024, 0, device,
                                       reducer.initialize(), num_preserved_vals, output);
                }

                LAUNCH_CUDA_KERNEL((OuterReductionKernel<num_per_thread, Self, Op, Index>), num_blocks, block_size, 0,
                                   device, reducer, self, num_coeffs_to_reduce, num_preserved_vals, output);
