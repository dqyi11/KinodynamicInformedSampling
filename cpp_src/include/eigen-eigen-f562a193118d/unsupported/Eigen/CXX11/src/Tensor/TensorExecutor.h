                ...
                if (needs_assign)
                {
                    ...
                    else
                    {
                        const Index PacketSize =
                            Vectorizable ? unpacket_traits<typename Evaluator::PacketReturnType>::size : 1;
                        ...
                        {
                            device.enqueue_with_barrier(&barrier, &EvalRange<Evaluator, Index, Vectorizable>::run,
...
// GPU: the evaluation of the expression is offloaded to a GPU.
        ...
        struct EigenMetaKernelEval<Evaluator, Index, true>
        {
            static __device__ EIGEN_ALWAYS_INLINE void run(Evaluator &eval, Index first, Index last, Index step_size)
            {
                ...
                // Use the vector path
                for (Index i = first * PacketSize; i < vectorized_size; i += vectorized_step_size)
                {
                    eval.evalPacket(i);
                }
        ...
        /*static*/
        template <typename Expression, bool Vectorizable>
        inline void TensorExecutor<Expression, GpuDevice, Vectorizable>::run(const Expression &expr,
                                                                             const GpuDevice &device)
        {
            ...
            if (needs_assign)
            {
                ...
                const int num_blocks =
                    numext::maxi<int>(numext::mini<int>(max_blocks, divup<int>(size, block_size)), 1);

                LAUNCH_CUDA_KERNEL((EigenMetaKernel<TensorEvaluator<Expression, GpuDevice>, Index>), num_blocks,
                                   block_size, 0, device, evaluator, size);
            }
            evaluator.cleanup();
        }
