...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        public:
                        ...
                        const array<Index, NumKernelDims> &indices)
            {
                ...
                const size_t offset =
                    static_cast<int>(Layout) == static_cast<int>(ColMajor) ? 0 : NumDims - NumKernelDims;
                for (int i = 0; i < NumKernelDims; ++i)
                {
        ...
        struct traits<TensorConvolutionOp<Dimensions, InputXprType, KernelXprType>>
        {
            ...
            // are different.
            typedef typename promote_storage_type<typename InputXprType::Scalar, typename KernelXprType::Scalar>::ret
                Scalar;
    ...
    public:
        ...
        EIGEN_DEVICE_FUNC
            EIGEN_STRONG_INLINE const typename internal::remove_all<typename KernelXprType::Nested>::type &
...
// possible.
    ...
    EigenConvolutionKernel1D(InputEvaluator eval,
                             ...
                             const float *__restrict kernel, const int numPlanes, const int numX, const int maxX,
                             const int kernelSize, float *buffer)
    {
...
#pragma unroll
            for (int i = threadIdx.x; i < num_x_input; i += blockDim.x)
            {
                const int tensor_index =
                    plane_input_offset + indexMapper.mapCudaInputKernelToTensorInputOffset(i + first_x);
...
#pragma unroll
                ...
                const int tensor_index =
                    plane_output_offset + indexMapper.mapCudaOutputKernelToTensorOutputOffset(i + first_x);
    ...
    __global__ void EigenConvolutionKernel2D(
        ...
        const float *__restrict kernel, const int numPlanes, const int numX, const int maxX, const int numY,
        const int maxY, const int kernelSizeX, const int kernelSizeY, float *buffer)
    {
        extern __shared__ float s[];
...
#pragma unroll
                for (int i = threadIdx.x; i < num_x_input; i += blockDim.x)
                {
                    const int tensor_index =
                        plane_input_offset +
                        indexMapper.mapCudaInputKernelToTensorInputOffset(i + first_x, j + first_y);
...
#pragma unroll
                    ...
                    const int tensor_index =
                        plane_output_offset +
                        indexMapper.mapCudaOutputKernelToTensorOutputOffset(i + first_x, j + first_y);
                    buffer[tensor_index] = result;
                }
            }
    ...
    struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice>
    {
          ...
          , m_device(device)
        {
            ...
            const typename TensorEvaluator<KernelArgType, GpuDevice>::Dimensions &kernel_dims =
        ...
        void executeEval(Scalar *data) const
        {
            ...
            switch (NumKernelDims)
            {
                case 1:
                {
                    ...
                    const int single_stride_dim = static_cast<int>(Layout) == static_cast<int>(ColMajor) ?
                                                      0 :
                    ...
                    switch (kernel_size)
                    {
                        case 4:
                        {
                            LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>,
                                                                         Index, InputDims, 4>),
                                               num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                               m_kernel, numP, numX, maxX, 4, data);
                            break;
                        }
                        case 7:
                        {
                            LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>,
                                                                         Index, InputDims, 7>),
                                               num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                               m_kernel, numP, numX, maxX, 7, data);
                            break;
                        }
                        default:
                        {
                            LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>,
                                               ...
                                               num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                               m_kernel, numP, numX, maxX, kernel_size, data);
                        }
                    }
                    break;
                }
                ...
                case 2:
                {
                    ...
                    const int maxP = numext::mini<int>(
                        maxSharedMem / ((kernel_size_x - 1 + maxX) * (kernel_size_y - 1 + maxY) * sizeof(Scalar)),
                        numP);
                    ...
                    dim3 num_blocks(num_x_blocks, num_y_blocks,
                    ...
                    switch (kernel_size_x)
                    {
                        case 4:
                        {
                            switch (kernel_size_y)
                            {
                                ...
                                {
                                    LAUNCH_CUDA_KERNEL(
                                        (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>, Index,
                                                                  InputDims, 4, 7>),
                                        num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                        m_kernel, numP, numX, maxX, numY, maxY, 4, 7, data);
                                    break;
                                }
                                default:
                                {
                                    LAUNCH_CUDA_KERNEL(
                                        (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>, Index,
                                                                  InputDims, 4, Dynamic>),
                                        num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                        m_kernel, numP, numX, maxX, numY, maxY, 4, kernel_size_y, data);
                                    break;
                                }
                            }
                            break;
                                ...
                                case 4:
                                {
                                    LAUNCH_CUDA_KERNEL(
                                        (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>, Index,
                                                                  InputDims, 7, 4>),
                                        num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                        m_kernel, numP, numX, maxX, numY, maxY, 7, 4, data);
                                    break;
                                }
                                default:
                                {
                                    LAUNCH_CUDA_KERNEL(
                                        (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>, Index,
                                                                  InputDims, 7, Dynamic>),
                                        num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                        m_kernel, numP, numX, maxX, numY, maxY, 7, kernel_size_y, data);
                                    break;
                                }
                            }
                            break;
                        ...
                        default:
                        {
                            LAUNCH_CUDA_KERNEL((EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>,
                                                                         Index, InputDims, Dynamic, Dynamic>),
                                               num_blocks, block_size, shared_mem, m_device, m_inputImpl, indexMapper,
                                               m_kernel, numP, numX, maxX, numY, maxY, kernel_size_x, kernel_size_y,
                                               data);
                            break;
                        }
                    }
                ...
                case 3:
                {
                    ...
                    const int maxZ = numext::mini<int>(
                        128, numext::mini<int>(maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1) *
