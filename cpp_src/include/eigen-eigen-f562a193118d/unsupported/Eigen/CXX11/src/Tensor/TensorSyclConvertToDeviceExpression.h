        ...
        namespace internal
        {
            ...
            struct NonOpConversion
            {
                typedef typename GetType<IsConst,
                                         NonOpCategory<typename ConvertToDeviceExpression<Args>::Type...>>::Type Type;
            };

            template <template <class, template <class> class> class NonOpCategory, bool IsConst, typename Args>
            struct DeviceConvertor
            {
                typedef typename GetType<IsConst, NonOpCategory<typename ConvertToDeviceExpression<Args>::Type,
...
#define TENSORMAPCONVERT(CVQual)                                                                                       \
    template <typename Scalar_, int Options_, int Options2_, int NumIndices_, typename IndexType_,                     \
...
#define KERNELBROKERCONVERTREDUCTION(CVQual)                                                                           \
    ...
    {                                                                                                                  \
        typedef CVQual                                                                                                 \
            TensorReductionOp<OP, Dim, typename ConvertToDeviceExpression<subExpr>::Type, MakeGlobalPointer> Type;     \
    };
