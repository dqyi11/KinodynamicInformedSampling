...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        struct are_inner_most_dims<ReducedDims, NumTensorDims, ColMajor>
        {
            ...
            static const bool tmp3 = index_statically_eq<ReducedDims>(array_size<ReducedDims>::value - 1,
                                                                      array_size<ReducedDims>::value - 1);
        ...
        {
            static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(const Self &self, typename Self::Index index,
                                                                     Op &reducer, typename Self::CoeffReturnType *accum)
            {
        ...
        struct FullReducerShard
        {
            ...
            {
                *output =
                    InnerMostDimReducer<Self, Op, Vectorizable>::reduce(self, firstIndex, numValuesToReduce, reducer);
            }
        };
        ...
        struct FullReducer<Self, Op, ThreadPoolDevice, Vectorizable>
        {
                            ...
                            typename Self::CoeffReturnType *output)
            {
                ...
                const int num_threads =
                    TensorCostModel<ThreadPoolDevice>::numThreads(num_coeffs, cost, device.numThreads());
                if (num_threads == 1)
                {
                    *output = InnerMostDimReducer<Self, Op, Vectorizable>::reduce(self, 0, num_coeffs, reducer);
                    return;
                ...
                {
                    device.enqueue_with_barrier(&barrier, &FullReducerShard<Self, Op, Vectorizable>::run, self,
    ...
    struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
    {
        ...
        evalSubExprsIfNeeded(typename MakePointer_<CoeffReturnType>::Type data)
        {
            ...
            else if (RunningOnSycl)
            {
                ...
                if (!data)
                {
                    data = static_cast<CoeffReturnType *>(
                        m_device.allocate(sizeof(CoeffReturnType) * num_coeffs_to_preserve));
                    m_result = data;
                }
                Op reducer(m_reducer);
            ...
            else if (RunningOnGPU && (m_device.majorDeviceVersion() >= 3))
            {
                    ...
                    (reducing_inner_dims || ReducingInnerMostDims))
                {
                    ...
                    Op reducer(m_reducer);
                    if (internal::InnerReducer<Self, Op, Device>::run(*this, reducer, m_device, data,
                ...
                if (internal::OuterReducer<Self, Op, Device>::HasOptimizedImplementation && preserving_inner_dims)
                {
                    ...
                    Op reducer(m_reducer);
                    if (internal::OuterReducer<Self, Op, Device>::run(*this, reducer, m_device, data,
                                                                      num_values_to_reduce, num_coeffs_to_preserve))
                    {
                        if (m_result)
                        {
        ...
        EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(Index index) const
        {
            ...
            if (ReducingInnerMostDims || RunningFullReduction)
            {
                ...
                return internal::InnerMostDimReducer<Self, Op>::reduce(*this, firstInput(index), num_values_to_reduce,
                                                                       reducer);
            }
            else
            {
