...
namespace Eigen
{
    namespace internal
    {
        ...
        struct tensor_static_symgroup_element_ctor
        {
            typedef tensor_static_symgroup_element<
                typename gen_numeric_list_swapped_pair<int, N, Gen::One, Gen::Two>::type, Gen::Flags> type;
        };
        ...
        public:
            static_assert(iia::count == iib::count, "Cannot multiply symmetry elements with different number of "
                                                    "indices.");
        ...
        struct tensor_static_symgroup_equality
        {
            ...
            constexpr static int ffa = A::flags;
            constexpr static int ffb = B::flags;
            static_assert(iia::count == iib::count, "Cannot compare symmetry elements with different number of "
                                                    "indices.");
        ...
        constexpr static inline std::array<Index, N> tensor_static_symgroup_index_permute(
            std::array<Index, N> idx, internal::numeric_list<int, ii...>, internal::numeric_list<int, jj...>)
        {
        ...
        struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>>
        {
            ...
            {
                static_assert(NumIndices >= SGNumIndices, "Can only apply symmetry group to objects that have at least "
                                                          "the required amount of indices.");
                typedef typename internal::gen_numeric_list<int, NumIndices - SGNumIndices, SGNumIndices>::type
                    remaining_indices;
                initial =
                    Op::run(tensor_static_symgroup_index_permute(idx, typename first::indices(), remaining_indices()),
                            first::flags, initial, std::forward<Args>(args)...);
                return tensor_static_symgroup_do_apply<internal::type_list<next...>>::template run<Op, RV,
                                                                                                   SGNumIndices>(
                    idx, initial, args...);
            }
    ...
    public:
        ...
        {
            return internal::tensor_static_symgroup_do_apply<ge>::template run<Op, RV, NumIndices>(idx, initial,
                                                                                                   args...);
        }
