...
namespace Eigen
{
    namespace internal
    {
        ...
        template <template <typename, typename> class test, typename check_against, typename h_list,
                  int default_flags = 0, bool last_check_positive = false, int last_check_flags = default_flags>
        ...
        template <template <typename, typename> class test,
                  typename check_against EIGEN_TPL_PP_SPEC_HACK_DEFC(typename, empty), int default_flags,
                  int last_check_flags>
        struct contained_in_list_gf<test, check_against, type_list<EIGEN_TPL_PP_SPEC_HACK_USE(empty)>, default_flags,
                                    false, last_check_flags>
        {
            constexpr static bool value = false;
        ...
        {
            template <typename A, typename B>
            EIGEN_DEVICE_FUNC constexpr static inline auto run(A a, B b) -> decltype(a + b)
            {
                return a + b;
            }
        ...
        constexpr inline array<decltype(Op::run(A(), B())), N> h_array_zip(array<A, N> a, array<B, N> b,
                                                                           numeric_list<int, n...>)
        {
        ...
        struct h_instantiate_by_c_array<InstType, ArrType, N, false, Ps...>
        {
            ...
            {
                return h_instantiate_by_c_array<InstType, ArrType, N - 1, false, Ps..., ArrType>::run(arr + 1, args...,
                                                                                                      arr[0]);
            }
        };
        ...
        struct h_instantiate_by_c_array<InstType, ArrType, N, true, Ps...>
        {
            static InstType run(ArrType *arr, Ps... args)
            {
                return h_instantiate_by_c_array<InstType, ArrType, N - 1, false, ArrType, Ps...>::run(arr + 1, arr[0],
                                                                                                      args...);
            }
        };
