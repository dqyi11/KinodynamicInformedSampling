...
namespace Eigen
{
    ...
    /** \class EulerAngles
      ...
      *represented
      *  by Euler angles, but there is no singular representation (e.g. unlike
      *rotation matrices).
      ...
      *
      * #### implicit ranges ####
      * When using implicit ranges, all angles are guarantee to be in the range
      *[-PI, +PI],
      *  unless you convert from some other Euler angles.
      * In this case, the range is __undefined__ (might be even less than -PI or
      *greater than +2*PI).
      ...
      *choose.
      * In the range Boolean parameter, you're been ask whether you prefer the
      *positive range or not:
    ...
    public:
        ...
        */
        ...
        /** Constructs and initialize Euler angles from a 3x3 rotation matrix \p m,
          ...
          *in [-PI, +PI].
          * \param positiveRangeBeta If true, beta will be in [0, 2*PI]. Otherwise, in
          *[-PI, +PI].
          * \param positiveRangeGamma If true, gamma will be in [0, 2*PI]. Otherwise,
          *in [-PI, +PI].
        */
        template <typename Derived>
        EulerAngles(const MatrixBase<Derived> &m, bool positiveRangeAlpha, bool positiveRangeBeta,
        ...
        */
        template <typename Derived>
        EulerAngles(const RotationBase<Derived, 3> &rot, bool positiveRangeAlpha, bool positiveRangeBeta,
                    bool positiveRangeGamma)
        {
            System::CalcEulerAngles(*this, rot.toRotationMatrix(), positiveRangeAlpha, positiveRangeBeta,
                                    positiveRangeGamma);
        }
        ...
        */
        ...
        /** Constructs and initialize Euler angles from a 3x3 rotation matrix \p m,
          ...
          * \tparam positiveRangeAlpha If true, alpha will be in [0, 2*PI]. Otherwise,
          *in [-PI, +PI].
          * \tparam positiveRangeBeta If true, beta will be in [0, 2*PI]. Otherwise,
          *in [-PI, +PI].
          * \tparam positiveRangeGamma If true, gamma will be in [0, 2*PI]. Otherwise,
          *in [-PI, +PI].
          */
