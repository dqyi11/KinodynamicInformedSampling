...
namespace Eigen
{
    namespace internal
    {
        ...
        void matrix_function_compute_cluster_size(const ListOfClusters &clusters,
                                                  Matrix<Index, Dynamic, 1> &clusterSize)
        {
            ...
            for (typename ListOfClusters::const_iterator cluster = clusters.begin(); cluster != clusters.end();
                 ++cluster)
            {
        ...
        /** \brief Solve a triangular Sylvester equation AX + XB = C
          ...
          */
        template <typename MatrixType>
        MatrixType matrix_function_solve_triangular_sylvester(const MatrixType &A, const MatrixType &B,
                                                              const MatrixType &C)
        {
                                                    ...
                                                    const VectorType &clusterSize, MatrixType &fT)
        {
            ...
            for (Index k = 1; k < clusterSize.rows(); k++)
            {
                for (Index i = 0; i < clusterSize.rows() - k; i++)
                {
                    ...
                    DynMatrixType B =
                        -T.block(blockStart(i + k), blockStart(i + k), clusterSize(i + k), clusterSize(i + k));
