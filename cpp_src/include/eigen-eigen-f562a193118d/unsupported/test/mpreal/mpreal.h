...
// MPFR 2.4.0 Specifics
        ...
        friend const mpreal urandom(gmp_randstate_t &state, mp_rnd_t rnd_mode);  // use gmp_randinit_default()
                                                                                 // to init state,
                                                                                 // gmp_randclear() to clear
#endif

#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
        friend const mpreal grandom(gmp_randstate_t &state, mp_rnd_t rnd_mode);  // use gmp_randinit_default()
                                                                                 // to init state,
                                                                                 // gmp_randclear() to clear
...
// MPFR < 2.4.2 Specifics
    ...
    // inline bool operator >= (const mpreal& a, const unsigned int b      ){
    // return !isnan EIGEN_NOT_A_MACRO (isnan()a) && (mpfr_cmp_ui(a.mpfr_srcptr(),b)
    // >= 0 );                }
    inline bool operator>=(const mpreal &a, const long int b)
    {
        return !isnan EIGEN_NOT_A_MACRO(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) >= 0);
