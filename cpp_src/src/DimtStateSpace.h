...
namespace ompl
{
	namespace base
	{
		...
		private:
			Dimt dimt_;
			DoubleIntegrator<PARAM_DOF> double_integrator_;	
		public:
			DimtStateSpace(Dimt dimt, DoubleIntegrator<PARAM_DOF> double_integrator, unsigned int dim = 0) : 
			...
			virtual double distance(const State *state1, const State *state2) const
			{
			  ...
			  DoubleIntegrator<PARAM_DOF>::StateVector eig_from, eig_to;
			  for (unsigned int i = 0; i < dimension_; ++i)
			  {
			  	eig_from[i] = s1[i];
			  	eig_to[i] = s2[i];
		  ...
		  virtual void interpolate(const State *from, const State *to, const double t, State *state) const
		  {
			  ...
			  DoubleIntegrator<PARAM_DOF>::StateVector eig_from, eig_to, eig_state;
			  // for (unsigned int i = 0 ; i < dimension_ ; ++i)
			  ...
			  for (unsigned int i = 0; i < dimension_; ++i)
			  {
				...
				DoubleIntegrator<PARAM_DOF>::Trajectory traj = double_integrator_.getTrajectory(eig_from, eig_to);
