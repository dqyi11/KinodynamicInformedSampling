...
MatrixXd GibbsSampler::sample(const int& no_samples,
  ...
  for (int i = 0; i < no_samples; i++) {
    ...
    do {
      ...
      sample = get_random_sample(min_vals[(i + skip) % dim],
                                 max_vals[(i + skip) % dim], (i + skip) % dim);
      trys++;
      // if (VERBOSE) std::cout << "Trys:" << trys << " Skip:" << skip <<
      // std::endl;
...
MatrixXd HitAndRun::sample(const int& no_samples,
                           high_resolution_clock::duration& duration) {
  // Get the limits of the space
  VectorXd max_vals, min_vals;
  const int dim = problem().start_state().size();
